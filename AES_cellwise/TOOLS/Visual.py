    ...
    for i in range(np.shape(lmat)[0]):
        if((lmat[i].sum())==2):# is nonlinear
            ...
            for j in range(32*(round_num+1)):
                ...
                if(lmat[i][j]>=1 and is_active(j)):
                    if(ind<16):
                        l_tmp=l_tmp+" + "+str(lmat[i][j])+" [x^"+str(int(rn))+"_"+str(ind)+"]"
                    else:
                        l_tmp=l_tmp+" + "+str(lmat[i][j])+" [y^"+str(int(rn))+"_"+str((ind-16))+ "]"
                elif(lmat[i][j]>=1):
                    if(ind<16):
                        l_tmp=l_tmp+" + "+str(lmat[i][j])+" x^"+str(int(rn))+"_"+str(ind)+""
                    else:
            ...
            for k in range(16*(round_num+1)):
                if(lmat[i][k+round_num*32+32]>=1):
                    ...
                    l_tmp=l_tmp+' +'+str(lmat[i][k+round_num*32+32])+' k_'+str(k//16)+"_"+str(k%16)
